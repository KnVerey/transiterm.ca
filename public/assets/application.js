...
;
(function() {
  var stallForImportResponse;

  jQuery(function() {
    $('#new_excel_import').submit(stallForImportResponse);
    return $('#upload-file').removeAttr('disabled');
  });

  stallForImportResponse = function() {
    $('#upload-file').attr('disabled', 'disabled');
    return $('#please-wait-modal').foundation('reveal', 'open');
  };

}).call(this);
(function() {
  var toggleRecordExpand;

  jQuery(function() {
    $('#notice, #alert, #success').slideFlash();
    $('#term-records').on('click', '[data-toggler]', toggleRecordExpand);
    return $('#delete-link').on('click', function() {
      return confirm("Are you sure? Deletion can't be undone!");
    });
  });

  $.fn.slideFlash = function() {
    return this.each(function() {
      $(this).animate({
        top: 30
      }, 800);
  ...
  toggleRecordExpand = function() {
    var $this;
    $this = $(this);
    $this.closest('tr').next('.record-expand').toggle();
    return $this.parent().children('[data-toggler]').toggle();
  };
...
;
(function() {
  var ajaxSearch, appendNextPage, bindInfiniteScroll, clearRecordsTable, setWaitMsg, userNearBottom;
  ...
  bindInfiniteScroll = function() {
    return $(window).scroll(function() {
      ...
      if ((url != null) && userNearBottom()) {
        return appendNextPage(url);
      }
    });
  };

  userNearBottom = function() {
    return $(window).scrollTop() > $(document).height() - $(window).height() - 350;
  };

  appendNextPage = function(url) {
  ...
  ajaxSearch = function(data, url) {
    ...
    return $.ajax({
      ...
      success: function() {
        $('.pagination').addClass('hide');
        return $('#records-table').show();
      }
    });
  };

  clearRecordsTable = function() {
    $('.pagination').empty();
    $('#records-table').hide();
    return $('#term-records').empty();
  };

  setWaitMsg = function(msg) {
    return $('#message-area').html("<i class='fa fa-spinner fa-spin'></i> &nbsp&nbsp" + msg);
  };
...
(function() {
  ...
  jQuery(function() {
    ...
    if ($('#term_record_domain_name').length) {
      return $('#term_record_domain_name').autocomplete({
        source: getDomains,
  ...
  getSources = function(request, response) {
    return $.ajax({
      url: '/sources',
      data: {
        filter: request.term
